...
pub fn get_string_from_memory(ptr: usize, len: usize, memory: Memory, store: StoreContext<String>) -> String {
    let mut buf = vec![0; len].into_boxed_slice();
    let _ = memory.read(store, ptr, &mut buf);
    String::from_utf8(buf.to_vec()).unwrap()
}

pub fn put_string_in_memory(content: &str, memory: Memory, store: StoreContextMut<String>) -> i32{
    let mut ptr = 0;
    unsafe {
